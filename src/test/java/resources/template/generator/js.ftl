//初始化 layui 数据表格
var prefix = "/sys/${baseRequestMapping}/";
$(function () {
    init();
})
    ...
    layui.use(['table', 'element'], function () {
        ...
        table.render({
            ...
            , cols: [[
                ...
                , {fixed: 'right', width: 180, align: 'center', toolbar: '#bar${modelNameUpperCamel}'}
            ]]
            , page: true
            , id: 'testReload'
        });
...
//增加页面
function ${modelNameLowerCamel}Add() {
    layer.open({
        type: 2
        , title: '增加字典类型'
        , content: prefix + '${modelNameLowerCamel}Add'
        , maxmin: true
        , area: ['650px', '550px']
        , btn: ['确定', '取消']
        , yes: function (index, layero) {
            var iframeWindow = window['layui-layer-iframe' + index]
                , submitID = '${modelNameLowerCamel}AddSubmit'
...
//编辑页面
function ${modelNameLowerCamel}Edit(id) {
    layer.open({
        type: 2
        , title: '修改字典信息'
        ...
        , yes: function (index, layero) {
            var iframeWindow = window['layui-layer-iframe' + index]
                , submitID = '${modelNameLowerCamel}UpdateSubmit'
                , submit = layero.find('iframe').contents().find('#' + submitID);
            //监听提交
            iframeWindow.layui.form.on('submit(' + submitID + ')', function (data) {
                var field = data.field; //获取提交的字段
